...
(def ^:private check-constraint
  ...
  (p/map-result (fn [[_ p ni?]] (pg-sqlt/->PgSqlCheckConstraint nil p ni? nil nil))
                (p/seq (p/lit :check)
                       to-sql
                       (p/map-result #(= :no-inherit %)
                                     (p/? (p/lit :no-inherit))))))
...
(def ^:private index-parameters
  ...
  (p/map-result #(if (empty? %) nil %)
                (p/seq)))

(def ^:private unique-table-constraint
  "Parser for a UNIQUE table constraint."
  (p/map-result (fn [[_ c ip]] (pg-sqlt/->PgSqlUniqueConstraint nil c ip nil nil))
                (p/seq (p/lit :unique)
                       columns
                       (p/? index-parameters))))
...
(def ^:private primary-key-table-constraint
  ...
  (p/map-result (fn [[_ c ip]] (pg-sqlt/->PgSqlPrimaryKeyConstraint nil c ip nil nil))
...
(def ^:private foreign-key-table-constraint
  ...
  (p/map-result (fn [[_ c _ t rc m od ou]]
...
(def ^:private column-definition
  ...
  (p/map-result #(apply pg-sqlt/->PgSqlColumnDefinition %)
                (p/seq to-sql
                       dt/type
                       (p/? (p/map-result #{:sith-options} (p/lit :with-options)))
                       (p/* column-constraint))))

(defn parse-row
  "Parses a row of a table DDL returning either the parsed row or the original row."
  [row]
  (let [result (p/parse (p/map-result butlast
